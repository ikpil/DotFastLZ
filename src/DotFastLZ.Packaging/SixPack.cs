...
namespace DotFastLZ.Packaging
{
    public static class SixPack
    {
        ...
        public static readonly byte[] SIXPACK_MAGIC = new byte[] { 137, (byte)'6', (byte)'P', (byte)'K', 13, 10, 26, 10 };
        ...
        public static bool DetectMagic(byte[] buffer, int offset)
        {
            ...
            for (int c = 0; c < 8; c++)
            {
                if (buffer[offset + c] != SIXPACK_MAGIC[c])
                {
                    return false;
                }
            }
        ...
        {
            f.Write(SIXPACK_MAGIC, 0, SIXPACK_MAGIC.Length);
        }
